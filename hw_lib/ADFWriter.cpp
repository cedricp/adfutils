...
RETCODE myReadSector ( struct AdfDevice * const dev,
                       ...
                       uint8_t * const          buf )
{
	ArduinoFloppyReader::ADFWriter* adfw = (ArduinoFloppyReader::ADFWriter*)dev->userdata;
	if(adfw->readSector(n, size, buf)){
		return RC_OK;
	}
    return RC_ERROR;
}
...
char *join_path(char *path, char *file) {
    char *newpath = (char*)malloc(strlen(path) + strlen(file) + 2);
    if (!newpath) {
    }
...
void print_entry(struct AdfEntry *e, char *path) {
    ...
    BOOL print_comment = true && e->comment && *e->comment;
...
void print_tree(struct AdfList *node, char *path)
{
    for (; node; node = node->next) {
        print_entry((struct AdfEntry *)node->content, path);
        if (node->subdir) {
...
bool ADFWriter::checkDisk()
{
	return m_device.checkForDisk(true) == DiagnosticResponse::drOK;
}
...
bool ADFWriter::getDirectoryList(std::string path, std::vector<std::string>& ret)
{
	ret.clear();

	if( m_device.checkForDisk(true) != DiagnosticResponse::drOK){
		m_device.closePort();
		return false;
	}

	adfEnvInitDefault();
	if (!(m_adfdev = adfMountDev("", TRUE, this))) {
        printf("Cannot mount adf device\n");
		m_adfdev = nullptr;
		return false;
    } else {
		if (!(m_adfvol = adfMount(m_adfdev, 0, TRUE))) {
			printf("Cannot mount adf filesystem\n");
			return false;
		}
	}
	...
	list = adfGetRDirEnt(m_adfvol, m_adfvol->curDirPtr, TRUE);
	node = list;

	for (; node; node = node->next) {
		AdfEntry* entry = (AdfEntry*)node->content;
		std::string name = entry->name;
		if (entry->type == ST_DIR){
			name += "/";
		}
		ret.push_back(name);
        //print_entry((struct AdfEntry *)node->content, path.c_str());

        // if (node->subdir) {
        //     struct AdfEntry *e = (struct AdfEntry *) node->content;
        //     char *newpath = join_path(path.c_str(), e->name);
        //     print_tree(node->subdir, newpath);
        //     free(newpath);
        // }
    }
...
void ADFWriter::closeDevice() {
	if (m_adfvol) adfUnMount(m_adfvol);
    if (m_adfdev) adfUnMountDev(m_adfdev);
	m_device.closePort();
}
...
bool ADFWriter::readSector(int sectorNum, int size, uint8_t* const buf)
{
	int currentTrack = (sectorNum / 11) / 2;
	int surfaceIndex = (sectorNum / 11) % 2;
	int sector = sectorNum % 11;

	//printf("Reading cyl %i idx %i sector %i\n", currentTrack, surfaceIndex, sector);
	...
	if (m_device.readCurrentTrack(data, sizeof(ArduinoFloppyReader::RawTrackDataDD), false) == DiagnosticResponse::drOK) {
		findSectors(data, false, currentTrack, surface, AMIGA_WORD_SYNC, track, false);
	} else {
		return false;
	}
	...
	memcpy(buf, (const char*)track.validSectors[sector].data, size);
	return true;
}
