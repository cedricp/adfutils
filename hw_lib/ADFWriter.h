...
namespace ArduinoFloppyReader {
	...
	class ADFWriter {
		...
		AdfDevice *m_adfdev = NULL;
    	AdfVolume *m_adfvol = NULL;
	...
	public:  
		...
		bool getDirectoryList(std::string path, std::vector<std::string>& ret);
		...
		bool readSector(int sectorNum, int size, uint8_t* const buf);
	};
